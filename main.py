...
class window():
    ...
    def createTableOfPochodzenie(self):
        if self.database is not None and self.createdInt3 == 1:
            ...
            app.addTable("tabela_pochodzeń", klasyLista, wrap="700", colspan=3, row =1)
            self.lista_class = []
            self.createdInt3 = 0
            klasyLista = []
            klasyDBNaglowek = self.database.cursor().execute(
                "select column_name from user_tab_cols where table_name = 'RASY' OR table_name = 'EFEKTY_RASOWE'")
            #print(klasyDBNaglowek)
            ...
            for row in klasyDB:
                ...
                for item in row:
                    tmp.append(item)
                klasyLista.append(tmp)
            app.addTable("tabela_pochodzeń1", klasyLista, wrap="700", colspan=3, row =2)
